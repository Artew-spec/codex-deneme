...
# Only the new spreadsheet is supported
DEFAULT_FILE = 'tablo4_01082025d.xls'
...
def parse_args():
    ...
    parser.add_argument('--file', default=DEFAULT_FILE,
                        help='path to Excel file (default: %s)' % DEFAULT_FILE)
...
def main():
    ...
    staff_cols = ['P.DR.', 'D.DR.', 'DR.ÖĞR.']
    for c in staff_cols:
        df[c] = pd.to_numeric(df[c], errors='coerce')
    df['teacher_total'] = df[staff_cols].fillna(0).sum(axis=1)
    df = df[(df['rank'] >= 13000) & (df['rank'] <= 30000)]
    df = df[df['region'].isin(ALLOWED_REGIONS)]
    result = df[['uni','PROGRAM ADI (2)','rank','city','region','priority','quota'] + staff_cols + ['teacher_total']].sort_values(['priority','rank'])
